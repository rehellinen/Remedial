                                ...
                                <form id="singcms-listorder">
                                    <table class="table table-hover table-striped singcms-table">
                                        ...
                                        <?php if(is_array($comment)): $i = 0; $__LIST__ = $comment;if( count($__LIST__)==0 ) : echo "" ;else: foreach($__LIST__ as $key=>$comment): $mod = ($i % 2 );++$i;?><tr>
                                                <td>
                                                    <input size="2" type="text" name="listorder[<?php echo ($comment["message_id"]); ?>]" value="<?php echo ($comment["listorder"]); ?>"/>
                                                </td>
                                                <td><?php echo ($comment["message_id"]); ?></td>
                                                <td><?php echo (boardType($comment["people_type"])); ?></td>
                                                <td><?php echo ($comment["name"]); ?></td>
                                                <td><a href="<?php echo ($comment["photo_url"]); ?>">点击查看</a> </td>
                                                <td>
                                                    <a id="rehe-detail"  attr-commentid2="<?php echo ($comment["message_id"]); ?>">
                                                    点击查看
                                                    </a>
                                                </td>
                                                <td><?php echo ($comment["tel"]); ?></td>
                                                <td><span attr-id="<?php echo ($comment["message_id"]); ?>" class="sing_cursor singcms-on-off" id="comment1-on-off">
                                                    <?php echo (getMessageType($comment["message_type"])); ?>
                                                    </span></td>
                                                <td>
                                                    <span class="pe-7s-close-circle "id="rehe-delete" attr-id="<?php echo ($comment["message_id"]); ?>" attr-message="删除"></span>
                                                </td>
