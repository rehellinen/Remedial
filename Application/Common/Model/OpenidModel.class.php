...
class OpenidModel extends Model
{
    ...
    public function updateStatusById($id , $type) {
        ...
        //echo $id;echo $type;exit;
        if($type == 2){
            $data['type'] = $type;
            $res = $this->_db->where('people_id='.$id.' AND type IN(-1,0)')->save($data);
            return $res;
        }
        if($type == 0){
            $data['type'] = $type;
            $res = $this->_db->where('people_id='.$id.' AND type IN(-1,2)')->save($data);
            return $res;
        }
    }
